...
local UnityEngineUI = require('UnityEngine.UI')
-- using VRC.SDKBase;
-- using VRC.Udon;
...
seats = {}
    ...
    -- public int gameId = 0;
gameId = boundObjects0

    -- public LayerMask tileLayer;
---@type UnityEngine.ScriptedLayerMask
tileLayer = nil
...
PACK_SIZE = {0, 1, 4, 4, 9} -- Probably not needed

    -- // game id, seat id, isShuffle packed into byte
    -- const int headerSize = 1;
headerSize=1

-- TODO: Convert all these arrays into TileStates. Dunno why Hiina thought this was simpler.
-- ---@class TileState
-- TileState = {}

    -- private int[] tileState = new int[136];

---Fuck writing `for` loops 136 times.
---@param n int
---@param value any
---@return table
function fillWith(n, value)
    a={}
    for i=1,n do
        a[i]=value
    end
...
arbitraryTilePositions=fillWith(136,UnityEngine.CreateVector3())
arbitraryTileRotations=fillWith(136,UnityEngine.CreateQuaternion())
...
tileDimensions=UnityEngine.CreateVector3(0.0375,0.05,0.032)
...
tenBou=fillWith(NUMBER_OF_TENBOU,nil)
    -- private Rigidbody[] tenbouRigidbodies = new Rigidbody[68];
tenBouRigidbodies=fillWith(NUMBER_OF_TENBOU,nil)
    -- private object[] tenbouVrcPickups = new object[68];
tenbouVrcPickups=fillWith(NUMBER_OF_TENBOU,nil) -- TODO: CVRPickup
    -- private int tenbouCursor;
tenbouCursor=fillWith(NUMBER_OF_TENBOU,nil)
    -- private Vector3[] tenbouLastKnownPos = new Vector3[68];
tenbouLastKnownPos=fillWith(NUMBER_OF_TENBOU,nil)
    -- private Quaternion[] tenbouLastKnownRot = new Quaternion[68];
tenbouLastKnownRot=fillWith(NUMBER_OF_TENBOU,nil)
    -- private float[] tenbouLastMoved = new float[68];
tenbouLastMoved=fillWith(NUMBER_OF_TENBOU,0.0)
    -- private float[] tenbouLastAcked = new float[68];
tenbouLastAcked=fillWith(NUMBER_OF_TENBOU,0.0)
...
maxSyncedStringSize = 105
    -- private const int maxPacketCharSize = maxSyncedStringSize * 2;
maxPacketCharSize = maxSyncedStringSize * 2
...
maxDataCharSize = ((maxPacketCharSize-headerCharSize)/8.0)*8
    -- private const int maxDataByteSize = maxDataCharSize / 8 * 7;
maxDataByteSize = (maxDataCharSize/8)*7
...
function Start()
    ...
    dealParent = transform.Find("Placements")
    ...
    for i=1,136 do
        ...
        tile = tileParent.GetChild(i).gameObject
            -- tiles[i] = tile;
        tiles[i]=tile
            -- tileTransforms[i] = tile.transform;
        tileTransforms[i]=tile.transform
            -- tileBoxColliders[i] = tile.GetComponent<BoxCollider>();
        tileBoxColliders[i] = tile.GetComponent('UnityEngine.BoxCollider')

            -- var rt = tile.GetComponent<RiichiTile>();
        rt = tile.GetLuaScriptByName("RiichiTile")
            -- riichiTiles[i] = rt;
